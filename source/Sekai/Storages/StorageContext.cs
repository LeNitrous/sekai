...
/// <summary>
/// A virtual storage where other <see cref="Storage"/>s can be mounted on.
/// </summary>
public class StorageContext : Storage
{
    ...
    public StorageContext(Storage? root = null)
    {
        if (root != null)
            Mount("/", root);
    }

    public void Mount(string basePath, Storage storage)
    {
        if (string.IsNullOrEmpty(basePath))
            throw new ArgumentException("Path cannot be null or empty", nameof(basePath));

        if (storages.ContainsKey(basePath))
            throw new InvalidOperationException("Storage is already mounted on this point.");
    ...
    private record struct ResolveResult(Storage IStorage, string Path);
}
