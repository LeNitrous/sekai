...
/// <summary>
/// A virtual storage where other <see cref="IStorage"/>s can be mounted on.
/// </summary>
public class StorageContext : FrameworkObject, IStorage
{
    private readonly Dictionary<string, IStorage> storages = new();

    public StorageContext(IStorage? root = null)
    {
        if (root != null)
            Mount("/", root);
    }
