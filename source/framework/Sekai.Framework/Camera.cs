...
public class Camera : Behavior
{
    ...
    /// </summary>
    public BoundingFrustum Frustum => new(ProjMatrix * ViewMatrix);
    ...
    public override void Update(double delta)
    {
        ...
        ViewMatrix = Matrix4x4.CreateLookAt(transform.Position, transform.Position + Vector3.Transform(-Vector3.UnitZ, transform.Rotation), Vector3.UnitY);
        ProjMatrix = Projection == CameraProjectionMode.Perspective
            ? Matrix4x4.CreatePerspectiveFieldOfView(MathUtil.DegreesToRadians(FieldOfView), Width / (float)Height, NearClipPlane, FarClipPlane)
            : Matrix4x4.CreateOrthographicOffCenter(0, Width, Height, 0, NearClipPlane, FarClipPlane);
    }
}
